...
- 根据仓库扫描初始化 `.duoshe/`；
- 生成 `CODEMAP.md`，包含 Mermaid 代码图、入口点、目录职责和热点文件；
- 通过 `duoshe guide` 或 `duoshe init --guided` 进入逐步问答；
...
| `duoshe init --guided` | 可用 | 初始化后进入问答，并写入 guided 记忆段。 |
| `duoshe guide` | 可用 | 新增或更新项目记忆里的引导信息。 |
